...
public class Level2 extends JFrame implements KeyListener {
	...
	public Level2(int input) {
		
		super("Arkanoid Game - Level 2");
		...
		for(i=0; i<row*column; i++){
			if(i == 0) {
				...
				myLabels[i] = new JLabel("LEVEL 2", JLabel.CENTER);
	...
	private void changeLayoutBall() {
		if(count > 30) {
			timer.cancel();
			setVisible(false);
			Level3 level3Area = new Level3(score);
			...
			if(up) {
				if(left) {
					if(iconEmpty.equals(myLabels[(yBall-1)*column+xBall].getIcon()) && iconEmpty.equals(myLabels[((yBall-1)*column+(xBall-1))].getIcon())) {
						...
						if(yellow.equals(myLabels[(yBall-1)*column+xBall].getIcon())) {
						...
						if(yellow.equals(myLabels[yBall*column+(xBall-1)].getIcon())) {
						...
						if(yellow.equals(myLabels[(yBall-1)*column+(xBall-1)].getIcon())) {
